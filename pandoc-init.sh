...
if [[ -z "$1" ]]; then
    _document_name="paper"
else
    _document_name="$1"
fi

md_template() {
    cat > "${_document_name}.md" << EOF
---
...
mk_template() {
# This is a 'here' document, featuring some ugly escaping
    cat > Makefile << EOF
VERSION = 0.0.0
...
PANDOC_HTML_FLAGS = --include-in-header=buttondown.css \\
    --template=html-version-number.template

all: \$(FILENAME)_v\$(VERSION).pdf \$(FILENAME)_v\$(VERSION).html \$(FILENAME)_v\$(VERSION).docx
...
$(printf '\t')pandoc $< \$(PANDOC_FLAGS) \$(PANDOC_PDF_FLAGS) -o \$@

%.html: \$(FILENAME).md \$(FILENAME).bib
$(printf '\t')pandoc $< \$(PANDOC_FLAGS) \$(PANDOC_HTML_FLAGS) -o \$@

%.docx: \$(FILENAME).md \$(FILENAME).bib
$(printf '\t')pandoc $< \$(PANDOC_FLAGS) -o \$@

cleanhtml:
$(printf '\t')rm \$(FILENAME)_v\$(VERSION).html

cleanpdf:
$(printf '\t')rm \$(FILENAME)_v\$(VERSION).pdf

cleandocx:
$(printf '\t')rm \$(FILENAME)_v\$(VERSION).docx
...
/*
    ...
    Edited by Andrew Turner, 2016-06-12
